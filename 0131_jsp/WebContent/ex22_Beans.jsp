...
<body>
<%--
	자바빈 : JSP에서 파라미터값을 저장할 목적으로 제공하는 저장공간
	Bean Object(빈객체) : Java Bean에 저장되어 있는 객체를 의미.
	1. 사용순서
		1) VO 만들기. (변수들의 이름은 파라미터의 이름과 같아야 함)
		2) 빈 객체 생성
		<jsp:useBean id="호출이름" class = "vo 클래스 위치" scope ="생명주기"></jsp:useBean>			
			scope : page(기본) < request < session < application
		3) <jsp:setProperty> 액션 태그를 사용하면 알아서 파라미터가 저장
		4) <jsp:getProperty property> 액션 태그를 사용하면 결과를 출력. 잘 사용되지 않음.	
 --%>
