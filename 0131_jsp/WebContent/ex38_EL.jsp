...
<body>
	...
	EL : JSP에서 사용 가능한 새로운 스크립트 언어
	     ...
	     - 보통 JSP가 가지고 있는 4대 속성을 활용
	     	생명주기(크기) : pageContext < request < session < application
 ...
 --%>
	...
	<%--  표현식 대신 사용하기
	<%= 변수 또는 계산식 또는 메소드 결과%> ==> ${변수 또는 계산식 또는 메소드}
	 --%>
	<h2>표현식 산술 연산 사용예제</h2>
	<h3>
		<li>15+7=<%=15 + 7%></li>
		<li>15-7=<%=15 - 7%></li>
		<li>15*7=<%=15 * 7%></li>
	...
	<!-- EL 사용되는 변수는 반드시 4대 속성에 저장되어 있어야함 -->
	<%
	...
	<%--  4대 속성이 같은 이름에 데이터를 저장하고 있으면 
  		우선순위:  pageContext > request > session > application 순으로 검색
  	--%>
	...
	<form method="post">
		<fieldset>
			...
			<p>
				반려동물 : <select name="animal" multiple size="4">
			...
			<input type="hidden" name="cPage" value="1"> <input
				type="button" value="보내기" onclick="send_go(this.form)">
		</fieldset>
	</form>
